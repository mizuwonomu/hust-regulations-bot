"""Entrypoint cho prepare, run và summarize của intervention A.

CLI là API công khai duy nhất mà orchestrator dual-suite dùng để điều khiển
sub-experiment candidate-pair 41/3.
"""
...
def validate_run(
    spec_path: Path | None,
    policies: list[str],
) -> tuple[dict[str, Path], CandidatePlan]:
    """Kiểm tra definition A và dựng plan mà không ghi gì xuống đĩa."""
    resolved_spec_path = Path(spec_path) if spec_path is not None else None
    spec = read_candidate_spec(resolved_spec_path)
...
def materialize_run(
    validated: tuple[dict[str, Path], CandidatePlan],
    output_dir: Path,
    policies: list[str],
) -> PrepareOutcome:
    """Ghi một run A prepared từ plan đã validate, trừ khi path đã tồn tại."""
    ...
    run_dir = create_candidate_run(output_dir, manifest, plan.traces)
    report = render_candidate_report(manifest, plan.traces, [], None)
...
def prepare_run(
    ...
    """Kiểm tra definition A và dựng run mới mà không tạo model client."""
    output_dir = Path(output_dir)
    if output_dir.exists():
        return PrepareOutcome(status="existing-result", run_dir=output_dir)
...
def _build_clients(manifest: CandidateManifest) -> dict[str, object]:
...
def _execute_run(run_dir: Path, manifest: CandidateManifest) -> None:
    _, cases, traces, results = load_candidate_run(run_dir)
    if results:
    ...
    if manifest.status not in {"prepared", "running"}:
        raise DiagnosticArtifactError(f"run status {manifest.status!r} cannot be executed")
...
def _save_summary(run_dir, manifest, cases, traces, results):
    summary = summarize_candidate_run(manifest, cases, traces, results)
    report = render_candidate_report(manifest, traces, results, summary)
    finalize_candidate_run(run_dir, manifest, summary, report)


def finalize_run(run_dir: Path) -> str | None:
    """Reload artifact và ghi trạng thái cuối suy ra từ coverage đã lưu."""
    try:
        manifest, cases, traces, results = load_candidate_run(run_dir)
...
def summarize_run(run_dir: Path) -> str:
    """Tính lại summary và report của A hoàn toàn offline."""
    manifest, cases, traces, results = load_candidate_run(run_dir)
...
def execute_run(run_dir: Path) -> None:
    """Chạy schedule của run A đã prepared, set running rồi append từng result."""
    manifest = load_candidate_run(run_dir)[0]
...
def _build_parser() -> argparse.ArgumentParser:
    """Tạo parser ba phase cho intervention A."""
    parser = argparse.ArgumentParser(description="Candidate-pair 41/3 position diagnostic runner")
...
def main(argv: list[str] | None = None) -> int:
    """Điều phối một command A và trả exit code đã quy ước."""
    ...
    try:
        ...
        return 130
    except (DiagnosticArtifactError, OSError, ValueError) as exc:
