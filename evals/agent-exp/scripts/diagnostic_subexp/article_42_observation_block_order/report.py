"""Render Markdown report tái lập được từ artifact của intervention B."""
...
def _block(title: str, body: str, *, language: str = "text") -> str:
    fence = "```"
    while fence in body:
        fence += "`"
    return f"#### {title}\n\n{fence}{language}\n{body}\n{fence}\n"
...
def _observation_sections(manifest: ObservationManifest, traces: list[ObservationInputTrace]) -> str:
    lines: list[str] = ["### Observation transformation evidence\n"]
    for trace in traces:
        if trace.original_observation is None or trace.transformed_observation is None:
            continue
        lines.append(f"#### Arm `{trace.arm_id}`\n")
        lines.append(_block("Original observation", trace.original_observation))
        lines.append(
            _block(f"Transformed observation ({trace.arm_id})", trace.transformed_observation)
        )
        lines.append("**Block movement**\n")
        lines.append(
        ...
        for block in trace.blocks:
            lines.append(
                f"| `{block.block_id}` | {block.before_position} | {block.after_position} "
                f"| {block.before_byte_span} | {block.after_byte_span} "
                f"| {block.referenced_candidates} |"
            )
        lines.append("")
        lines.append("**Line-origin map**\n")
        lines.append("| output line | original line | segment | line sha256 |")
        lines.append("| --- | --- | --- | --- |")
        for item in trace.line_origin_map:
            lines.append(
                f"| {item.output_line_number} | {item.original_line_number} "
                f"| `{item.segment_id}` | `{item.line_sha256[:16]}` |"
            )
        lines.append("")
        lines.append("**Unchanged sections**\n")
        lines.append("| section | source bytes | output bytes | sha256 | referenced candidates |")
        lines.append("| --- | --- | --- | --- | --- |")
        for section in trace.unchanged_context_sections:
            lines.append(
                f"| `{section.section_id}` | {section.source_byte_span} "
                f"| {section.output_byte_span} | `{section.text_sha256[:16]}` "
                f"| {section.referenced_candidates} |"
            )
        lines.append("")
        if trace.page_split_group is not None:
            group = trace.page_split_group
            lines.append("**Article 41/3 page-split group**\n")
            lines.append(
                f"- group `{group.group_id}` stays inside block `{group.block_id}`\n"
                f"- original lines {group.before_line_numbers} -> output lines "
                f"{group.after_line_numbers}\n"
                f"- line hashes {group.line_sha256}\n"
            )
        lines.append(
            f"Declared changed fields: {trace.declared_changed_fields}; "
            f"verified unchanged fields: {trace.verified_unchanged_fields}\n"
        )
    return "\n".join(lines)


def _schedule_section(manifest: ObservationManifest, traces: dict[str, ObservationInputTrace]) -> str:
    ...
    for arm_id in seen:
        ...
        lines.append(
            ...
            f"| {trace.after_block_order or 'original'} "
...
def _metrics_section(summary: ObservationSummary) -> str:
    lines = ["### Metrics\n"]
    lines.append(
    ...
    lines.append("")
    if summary.observation_block_order is not None:
        lines.extend(_observation_metrics(summary.observation_block_order))
    return "\n".join(lines)
...
def _observation_metrics(outputs: ObservationOutputs) -> list[str]:
    lines = ["**Intervention B slices**\n"]
    lines.extend(_comparison_table(outputs.transitions, "Original-versus-reversed transitions"))
    lines.append(f"- {outputs.shared_block_note}\n")
    lines.append(f"- transformation trace identity: {outputs.observation_transform_trace_ids}\n")
    lines.append("**Selected candidate supporting evidence**\n")
    lines.append(
        "| policy | arm | batch | repeat | selected article | block ids | 1-based block positions |"
    )
    lines.append("| --- | --- | --- | --- | --- | --- | --- |")
    for row in outputs.selected_blocks:
        lines.append(
            f"| {row.policy} | `{row.arm_id}` | {row.batch_id} | {row.repeat_id} "
            f"| {row.selected_article} | {row.block_ids} | {row.block_positions} |"
        )
    lines.append("")
    return lines
...
def render_observation_report(
    ...
    """Render report Markdown của B, giữ nguyên observation gốc dạng literal block."""
    ...
    if any(trace.original_observation is not None for trace in traces):
        lines.append("## Observation evidence\n")
        lines.append(_observation_sections(manifest, traces))
    lines.append("## Decisions\n")
    lines.append(_decision_section(manifest, trace_by_id, results))
...
__all__ = ["render_observation_report"]
