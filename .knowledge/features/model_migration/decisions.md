...
## 2026-08-31 — Multi-hop hop-recall harness + first single-pass baseline

Extends job 3's eval revival with a metric the RAGAS suite cannot express. Commit 5f4fcef, branch harness/cross-reference.

**Chosen approach + why.** hop-recall is a deterministic, self-computed metric (plain set arithmetic on article numbers), NOT a RAGAS metric. Reason: "did retrieval pull the second cited article into the parent set" is a domain-specific set-membership question the library has no metric for; computing it ourselves makes it judge-free and epoch-independent, so it can serve as the honest retrieval gate while RAGAS context_recall stays the fuzzy, LLM-judged diagnostic. The two run side by side (never one replacing the other) and cross-check: hop_recall=1 with low RAGAS recall flags a gap between "fetched the article" and "the article actually answered the question". Because nothing in a library backs a self-computed metric, its pure-function unit tests ARE the correctness anchor - that is why parse_link/compute_hop_recall were split out as pure and tested first.

**second_hop is fixed to B, but B is the wrong thing to gate on - read full_recall as primary.** The corpus `link "A -> B"` encodes semantic intent (A = the article the question is framed on, B = the cited article), not retrieval difficulty. The baseline proved the two diverge: id=2 (link 22->13) and id=8 (link 42->40) scored second_hop_hit=True by retrieving B while MISSING A, because the query's lexical focus landed on B. So second_hop_hit (fixed to B) reports false wins; full_recall (both gold articles present) is the honest measure. Decision agreed with the user: promote full_recall to primary and redefine second_hop_hit as "the gold article single-pass dropped - was it recovered" (dynamic per row). NOT YET IN CODE - the committed harness still reports fixed-B second_hop_hit per group; the redefinition is debt (see tracker).

**Regression guard is the baseline full_recall=1.0 set, not the Control label.** Only id=1 (Treatment) and id=6 (Control) retrieve both gold articles single-pass. Those two - defined by measured outcome, not by group name - are the "agent must not regress" set.

**Control's design premise is falsified by the baseline.** The corpus author built Control as intra-chapter internal links meant to be single-pass-solvable ("0 regress" check). The baseline shows single-pass fails 2/3 Control (id=7 link 15->12, id=8 link 42->40 each drop one gold article). Implication: citation-following fails even WITHIN one chapter, not only on cross-degree edges - so the Treatment/Treatment yếu/Control labels are a difficulty/edge-type axis, not a solved/unsolved axis. This cross-validates the ratio-era finding (tracker: "6/8 multi-hop miss a gold article at 0.45") from a second angle and re-confirms it is a retrieval-recall problem, not degree filtering (see features/cross_reference).

**Baseline (single-pass, n=8, ratio 0.45, 2026-08-31):** per-group second_hop_hit Treatment 0.67, Treatment yếu 0.00, Control 0.67; full_recall shows 6/8 questions retrieve exactly ONE of the two gold articles (only id=1, id=6 get both, none get zero). The missing article is B in 4 rows (id 3,4,5,7) and A in 2 rows (id 2,8).

**Assumptions it rests on.** (1) The harness measures the SINGLE-PASS chain - it is the "before" baseline; Δ requires swapping the retrieval mirror to call the agent later. (2) n=2/3 per group means per-group rates are per-question stories, not stable percentages - always read the per-question rows; the corpus is 8 self-authored questions (weak statistically, flagged since the ratio work). (3) Parent article identity is stable from the splitter's title format.

**Tried / failed (dead-ends worth recording):**
- Tried: assume parent Documents carry `dieu`/`chuong` like children (spec step 2). Failed because: parents (pickled in doc_store_pdr) carry NO `dieu` - only a capital `Điều` metadata key holding the title string, and the doc preamble has `Điều: None`. Avoid when: reading any parent-level metadata field and assuming child metadata carries over - verify `parent.metadata.keys()` first. Fix: read the `Điều` key, fall back to a `Điều\s+(\d+)` regex on the title (both machine-generated by the splitter, not fuzzy text mining).
- Tried: use the group label as the regression guard / treat second_hop_hit as the primary gate. Failed because: Control isn't single-pass-solvable and second_hop_hit overstates (above). Avoid when: tempted to trust a corpus's design-time labels over its measured baseline.

**Forward decision (next feature, not built here): agent = pattern B.** The multi-hop citation-following agent will be autonomous-reasoning (reads retrieved article text, infers the citation to follow, issues the next retrieval itself), NOT a graph-backbone agent that follows cross_references.json edges deterministically (pattern A). Reasons: the user's goal is to learn/hand-write a real agent loop; scoring a graph-using agent against gold derived from that same graph is near-circular and uninformative; the deterministic graph backbone can be bolted on later as reliability, whereas the reasoning path cannot be retrofitted. cross_references.json is reserved for three non-input roles: offline scoring oracle, trajectory-debug comparator, later reliability fallback. This still needs its own brainstorm.
