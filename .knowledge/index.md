...
- **Groq models are retired without notice** — `qwen/qwen3-32b` was decommissioned mid-project and forced an unplanned answer-model swap. Every Groq model id in this repo (router, query rewrite, chitchat, answer generation, RAGAS judge) is a live external dependency that can disappear; a model choice here is never freely reversible, because the model you would revert *to* may no longer serve requests. See `features/model_migration/decisions.md`.
